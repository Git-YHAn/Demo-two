...
public class AsyncRestartTask {
	 ...
	 */
	@Async
	public void asyncRestartApp(Long appEnvId, Long serverId) throws UnsupportedEncodingException {
		...
		try {
			...
			try {
				if (application.getAppTypeId() == OpsBaseAppType.APP_TYPE_WEB) {
					...
					String cmd = "sh " + shellPath + " restart ";
					Channel channel = shell.exec(cmd);
					int result = shell.getExitStatus(channel);
					if (result > 0) {
						throw new RuntimeException("重启脚本执行失败: ERROR-STATUS " + result + ", cmd: " + cmd);
					}
				} else if(application.getAppTypeId() == OpsBaseAppType.APP_TYPE_JAVA){
					String startShellName = "startup.sh";
					String startShellPath = "/usr/local/action_" + appEnv.getAppName() + "/" + startShellName;
					...
					if (result > 0) {
						throw new RuntimeException("启动脚本执行失败: ERROR-STATUS " + result);
					}
					channel.disconnect();
				...
				throw new RuntimeException("应用【" + appEnv.getAppName() + "@" + server.getAssetsName() + "】重启失败：" + e.getMessage());
			}
		} finally {
			session.disconnect();
		}
	...
	public boolean restartApp(Long appEnvId, Long serverId){
		OpsBaseServer server = opsBaseServerDao.getById(serverId);
		OpsAssembleApp appEnv = opsAssembleAppDao.getByappEnvId(appEnvId);
		Shell shell = new Shell(server.getSshAddress(), server.getHostAccount(), server.getHostPassword(), server.getSshPort());
		Session session = shell.initSession();
