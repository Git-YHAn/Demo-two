<template>
<el-dialog title="重启服务器" :visible.sync="visible" @close="reset">
  <span>是否确定重启以下{{instances.length}}台服务器？</span>
...
export default {
  ...
  methods: {
    ...mapActions('action', ['rebootKsyunKecInstance']),
    async confirm() {
      this.loading = true
      let instanceIds = this.instances.map(item => {
        return item.instanceId
      })
      let response = await this.rebootKsyunKecInstance(instanceIds)
      if (response.code === '1') {
